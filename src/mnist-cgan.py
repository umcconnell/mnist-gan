...
# 
# In the first the discriminator is trained. The discriminator is fed some real samples from the training set, labelled as such (usually with a 1), and their corresponding label, such as `8`. Additionally, some fake samples generated by the discriminator, labelled as fake samples (usually with a 0), and their corresponding fake label, such as `3`, are fed to the discriminator. Then, standard backpropagation is applied onto the discriminator only.
# 
# ![Step 1: Training the discriminator](../static/cgan/figure1.png)

# #### Step 2: Training the Generator
# 
# Next, the generator is trained. First, the generator generates a batch of fake images with random noise and fake digit labels. The fake images and their corresponding are fed to the discriminator with real labels (usually a `1`). The error is then backpropagated to the generator, but only the generator weights are updated.
# 
